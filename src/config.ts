...
import { getLastGitTag, getPreGitRef } from './git'
...
export function defineChangeConfig(config: Partial<ChangelogConfig>) {
  return config
}
...
export async function resolveChangelogConfig(
  ...
  if (!config.from)
    config.from = await getPreGitRef()

  if (!config.to)
    config.to = await getLastGitTag()
